...
#SBATCH --nodes=1
#SBATCH --mem=64gb
#SBATCH --output=log/remo25_%j.out
#SBATCH --error=log/remo25_%j.out
#SBATCH --job-name=ReMo25
...
# ============================================================
# HARNESS + ReMo RUN — qwen25 only
#
#   System: HARNESS+REMO — run_monthly_baseline_remo
#   Model:  qwen25 — Qwen2.5-32B-Instruct-AWQ  (TP=1, 1×A100)
#   RUN_TAG — names remo_results_<tag>/ directory
# ============================================================
RUN_TAG="remo_v1"   # <-- keep in sync with run_remo_qwen36.sh
...
echo "=========================================="
echo "HARNESS + ReMo RUN — qwen25  [tag: ${RUN_TAG}]"
echo "=========================================="
echo "System:     HARNESS+REMO (plain reward model, raw returns)"
echo "Model:      qwen25 (Qwen2.5-32B-AWQ)  —  1×A100, TP=1"
...
# ============================================================
# MAIN LOOP — qwen25 × all universes
# ============================================================
...
echo ""
echo "=========================================="
echo "Generating qwen25 per-universe plots..."
echo "=========================================="
...
echo ""
echo "=========================================="
echo "HARNESS + ReMo qwen25 COMPLETE  [tag: ${RUN_TAG}]"
echo "=========================================="
echo "Period:  ${START_DATE} -> ${END_DATE}"
echo "Capital: \$${INITIAL_CAPITAL}  |  Fees: 15 bps"
echo ""
echo "Result directories (remo_results_${RUN_TAG}/qwen25_*):"
ls -lhd "${ROOT_DIR}/remo_results_${RUN_TAG}"/qwen25_*/ 2>/dev/null | awk '{print "  "$NF}' \
    || echo "  (none)"
echo "=========================================="
