"""
Runs the harness + AdaReMo system: conviction risk harness + adaptive reward
model only. No memory, no debater. Intended for a clean ablation against the
standard baseline (harness only) to isolate AdaReMo's effect.
...
def run_baseline_adamo(tickers, start, end, output_dir, initial_capital=1_000_000.0,
                       ...
                       adamo_alpha=1.0, adamo_min_samples=2):
    ...
    decision_fn = harnesses.make_baseline_adamo(
...
def main():
    parser = argparse.ArgumentParser(description="Run harness + AdaReMo backtest (no memory, no debater)")
    ...
    parser.add_argument("--adamo_min_samples", type=int, default=2,
                        help="minimum periods before AdaReMo signal activates (default 2 = fast activation)")
    ...
    run_baseline_adamo(
        ...
        adamo_alpha=args.adamo_alpha, adamo_min_samples=args.adamo_min_samples,
    )
