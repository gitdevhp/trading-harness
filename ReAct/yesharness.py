...
def _clean_download(df, ticker):
    """
    Convert yfinance output into:

        DatetimeIndex
        Close column

    Handles MultiIndex output, timezone-aware indexes,
    duplicate dates, empty downloads, and malformed data.
    """
    ...
    # --------------------------------------------------------
    # Handle MultiIndex output
    # --------------------------------------------------------

    if isinstance(df.columns, pd.MultiIndex):
        close_series = None

        # Generic search for Close regardless of level.
        ...
        if close_series is None:
            raise ValueError(
                f"{ticker}: MultiIndex download has no usable Close."
            )
...
def _download_ticker(
    ...
    """
    Retry yfinance several times and use Ticker.history()
    as a fallback.

    This specifically protects the backtest from temporary
    Yahoo Finance / crumb / request / MultiIndex failures.
    """
    ...
    for attempt in range(2):
        try:
            ...
            print(
                f"[DATA] {ticker}: history fallback "
    ...
    raise ValueError(
        f"{ticker}: unable to obtain usable market "
        f"data after all attempts. "
...
def prefetch_data(
    ...
    """
    Download one year of extra history before the requested
    period. This gives the harness enough data for 200-day
    indicators on the first rebalance.

    Every ticker must succeed. No asset is silently removed.
    """
    ...
    if failures:
        raise RuntimeError(
            "\n[DATA] Failed to load required assets:\n"
            + "\n".join(
                f"  - {failure}"
    ...
    print(
        f"[DATA] Loaded "
        f"{len(GLOBAL_DATA_CACHE)}/"
        f"{len(RAW_UNIVERSE)} assets successfully."
    )
    ...
    for asset in ANONYMOUS_UNIVERSE:
        ...
        if not dates:
            raise ValueError(
                ...
                f"no usable trading dates in requested period."
            )
...
def build_market_snapshot(
    ...
    for asset in ANONYMOUS_UNIVERSE:
        ...
        if closes.empty:
            raise ValueError(
                f"{REVERSE_MAP.get(asset, asset)}: "
                f"no prices through {current_date}."
            )
...
def apply_simple_harness(
    ...
    for asset in ANONYMOUS_UNIVERSE:
        ...
        if closes.empty:
            raise ValueError(
                f"{REVERSE_MAP.get(asset, asset)}: "
                f"no data available through {current_date}."
            )
...
def DECISION_FUNCTION(
    ...
    # Keep the ReAct system identical to the no-harness version.
    system = f"""You are an autonomous ReAct Portfolio Manager.
...
Use the tools to gather information before making the portfolio decision.
...
- Make your decision using the information available through the tools.
...
"""
    ...
    for _ in range(4):
        # Do not resend the entire raw market snapshot on every ReAct
        # turn. The snapshot can be large enough to push Qwen beyond
        # its 32,768-token context window. Keep only the system/user
        # prompt plus the latest assistant/observation pair.
        request_messages = [
            messages[0],
            messages[1],
        ]
            ...
            messages.append(
                {
                    ...
                    "content": (
                        ...
                        "Return valid asset percentage allocations "
                        "using only the provided assets and CASH."
                    ),
                }
            )
            continue
    ...
    # Keep the harness exactly as before.
    return apply_simple_harness(
        raw,
        current_date,
    )
...
VARIANT_NAME = "react_harness"

DEFAULT_OUTPUT = (
    "react_harness_results_compare.json"
)
